...
interface FilterPopupProps {
  onFiltersChange: (filters: { statuses: string[]; categories: string[]; priorities: string[] }) => void;
  initialFilters?: {
    statuses: string[];
    categories: string[];
    priorities: string[];
...
export default function FilterPopup({ onFiltersChange, initialFilters }: FilterPopupProps) {
  ...
  const hasActiveFilters =
    selectedStatuses.length > 0 || selectedCategories.length > 0 || selectedPriorities.length > 0;
      ...
      <PopoverTrigger asChild>
        <Button
          ...
          className={`relative flex items-center gap-2 px-3 py-2 h-auto border 
                        text-gray-700 dark:text-gray-200 hover:text-gray-800 dark:hover:text-gray-100 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
        ...
        >
          ...
          {hasActiveFilters && (
            <Badge className="ml-1 px-1.5 py-0.5 text-xs bg-blue-600 text-white">{totalActiveFilters}</Badge>
          )}
        </Button>
      </PopoverTrigger>
                ...
                <div key={priority.id} className="flex items-center space-x-2">
                  ...
                  >
                    <div
                      className="w-2 h-2 rounded-full"
                      style={{
                        backgroundColor: PRIORITY_COLORS[priority.name.toUpperCase() as keyof typeof PRIORITY_COLORS],
                      }}
                    />
