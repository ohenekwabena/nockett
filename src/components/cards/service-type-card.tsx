      ...
      <Card className="bg-white dark:bg-gray-800 shadow-md border-none min-w-[300px]">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold text-gray-900 dark:text-gray-100">Service Types</CardTitle>
            <button
        ...
        <CardContent className="pt-0">
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {serviceTypes.length === 0 ? (
              ...
              serviceTypes.slice(0, 8).map((serviceType) => (
                <div
                  key={serviceType.id}
                  className="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-700 rounded-md"
                >
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{serviceType.name}</span>
          ...
          {serviceTypes.length > 8 && (
            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
              <div className="text-xs text-gray-500 dark:text-gray-400 text-center">
                +{serviceTypes.length - 8} more service types
              </div>
            </div>
          )}
        </CardContent>
