...
interface DemarcationModalProps {
  ...
  onDemarcationsChange: () => void;
}

export default function DemarcationModal({ isOpen, onOpenChange, onDemarcationsChange }: DemarcationModalProps) {
  const [demarcations, setDemarcations] = useState<Demarcation[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editingName, setEditingName] = useState("");
  const [newDemarcationName, setNewDemarcationName] = useState("");
  ...
  const handleCreate = async () => {
    if (!newDemarcationName.trim()) {
      toast.error("Demarcation name is required");
      return;
    }
  ...
  const handleEdit = (demarcation: Demarcation) => {
    setEditingId(demarcation.id);
    setEditingName(demarcation.name);
  };
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl max-h-[80vh] bg-white dark:bg-gray-800 border-0 rounded-lg shadow-lg text-gray-800 dark:text-gray-200">
          <DialogHeader className="mt-4 px-2">
            <DialogTitle className="text-2xl dark:text-gray-200 text-gray-950">Manage Demarcations</DialogTitle>
            <DialogDescription className="text-gray-600 dark:text-gray-400">
              Create, edit, and delete demarcations.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-6 px-2">
            <div className="space-y-2">
              <Label htmlFor="new-demarcation" className="font-semibold mb-1 dark:text-gray-400 text-gray-600">
                Create New Demarcation
              </Label>
              <div className="flex gap-2 mt-1">
                <Input
                  id="new-demarcation"
                  placeholder="Enter demarcation name"
                  value={newDemarcationName}
                  onChange={(e) => setNewDemarcationName(e.target.value)}
                ...
                <button
                  ...
                  disabled={isCreating || !newDemarcationName.trim()}
            ...
            <div className="space-y-2">
              <Label className="text-sm font-semibold dark:text-gray-400 text-gray-600">Existing Demarcations</Label>
              <div className="border border-gray-300 dark:border-gray-600 rounded-md max-h-96 overflow-y-auto bg-gray-50 dark:bg-gray-700 mt-1">
                {isLoading ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="text-sm text-gray-500 dark:text-gray-400">Loading demarcations...</div>
                  </div>
                ) : demarcations.length === 0 ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="text-sm text-gray-500 dark:text-gray-400">No demarcations found</div>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          {editingId === demarcation.id ? (
                            ...
                            <span className="font-medium text-gray-900 dark:text-gray-100">{demarcation.name}</span>
                          )}
                        </div>
                          ...
                          {editingId === demarcation.id ? (
                            ...
                            <>
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => handleEdit(demarcation)}
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => setDeleteId(demarcation.id)}
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="bg-white dark:bg-gray-800 border-0 text-gray-800 dark:text-gray-200">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-gray-950 dark:text-gray-200">Delete Demarcation</AlertDialogTitle>
            <AlertDialogDescription className="text-gray-600 dark:text-gray-400">
              Are you sure you want to delete this demarcation? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
